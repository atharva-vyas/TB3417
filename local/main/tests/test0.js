...
let TIMESTAMP = web3.eth.getBlockNumber().then(async (result) => {
  web3.eth.getBlock(result).then(async (result) => {
    await result
...
const smartContract = "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D";
...
let TO = web3.eth.getAccounts().then(async (result) => {
  await result
  return result[0]
})

const PATH = [TOKEN_IN, '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', TOKEN_OUT]
...
web3.eth.getBlockNumber().then(async (timestamp0) => {
  web3.eth.getBlock('latest').then(async (timestamp) => {
    web3.eth.getAccounts().then((toAddr) => {      

      expiry = parseInt(timestamp.timestamp) + 300
      // console.log(1, AMOUNT_IN);
      // console.log(2, AMOUNT_OUT_MIN);
      // console.log(3, PATH);
      // console.log(4, toAddr[0]);

      web3.eth.getAccounts().then(async (acc) => {
        contract.methods.swapExactTokensForTokens(AMOUNT_IN, AMOUNT_OUT_MIN, PATH, toAddr[0], expiry).send({ from: DAI_WHALE }).then((result0) => {
...
//   contract.methods.swapExactTokensForTokens(AMOUNT_IN, AMOUNT_OUT_MIN, PATH, TO, TIMESTAMP).send({from: DAI_WHALE, gas:3000000}).then((result0) => {
//     console.log(JSON.stringify(result0));
//   }).catch((err0) => {
//     console.log(err0);
//   });
