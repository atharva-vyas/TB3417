            ...
            contractToken1.methods.decimals().call().then((result1) => {
                contractPool.methods.tickSpacing().call().then((tickSpacing0) => {
                    ...
                    function decimalToken0() {
                        ...
                        return (new BN(decimals).div(new BN(deicmal0))).mul(new BN(amountDeposit)).toString()
                    }
                    ...
                    price([poolABI, pool]).then((priceQuote) => {
                        ...
                        if (priceToTick(_OneToken0EqualsTo_Token1)<=main.tickLower || priceToTick(_OneToken1EqualsTo_Token0)<=main.tickLower) {
                            ...
                            if (distanceFrom_OneToken1EqualsTo_Token0 < distanceFrom_OneToken0EqualsTo_Token1) {
                                percentUpper = parseInt(((param.maxPrice / _OneToken1EqualsTo_Token0) * 100) - 100)
                                percentLower = parseInt(((param.minPrice / _OneToken1EqualsTo_Token0) * 100) - 100)

                                newMaxPrice = ((percentUpper / 100) * _OneToken0EqualsTo_Token1) + _OneToken0EqualsTo_Token1
                                newMinPrice = ((percentLower / 100) * _OneToken0EqualsTo_Token1) + _OneToken0EqualsTo_Token1
                            } else if (distanceFrom_OneToken1EqualsTo_Token0 > distanceFrom_OneToken0EqualsTo_Token1) {
                                percentUpper = parseInt(((param.maxPrice / _OneToken0EqualsTo_Token1) * 100) - 100)
                                percentLower = parseInt(((param.minPrice / _OneToken0EqualsTo_Token1) * 100) - 100)

                                newMaxPrice = ((percentUpper / 100) * _OneToken1EqualsTo_Token0) + _OneToken1EqualsTo_Token0
                                newMinPrice = ((percentLower / 100) * _OneToken1EqualsTo_Token0) + _OneToken1EqualsTo_Token0
                            }
                            ...
                            const main0 = {
                                ...
                                token1: param.tokenPair1,
                                fee: parseFloat(param.feeTier) * 10000,
                                tickUpper: priceToTick(newMaxPrice),
                                tickLower: priceToTick(newMinPrice),
                                amount0Desired: decimalToken0(),
                                amount1Desired: decimalToken1(),
                                amount0Min: tolerance(decimalToken0()),
                                amount1Min: tolerance(decimalToken1()),
...
mintMath({
    ...
    //     mint([result.token0, result.token1, result.fee, result.tickLower, result.tickUpper, result.amount0Desired, result.amount1Desired, result.amount0Min, result.amount1Min, result.recipient, result.deadline]).then((result1) => {
    //         console.log(result);
    //         console.log(result0);
