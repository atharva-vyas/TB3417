...
const V3poolC = new web3.eth.Contract(poolABI, pool);
async function dambe() {
    var S0 = await V3poolC.methods.slot0().call().catch(function (err){log(err)});
    var CT = parseInt(S0.tick);
    var Tsp = parseInt(await V3poolC.methods.tickSpacing().call().catch(function (err){log(err)}));

    // console.log(Math.floor(CT/Tsp));
...
async function faggot() {
    // sqrtPriceX96 = sqrt(1.0001^tick) * 2^96
    // price = sqrtPriceX96^2 / 2^96
    var S0 = await V3poolC.methods.slot0().call().catch(function (err){log(err)});
    // var CT = (new BN(S0.sqrtPriceX96).pow(new BN(2))).div(new BN(2).pow(new BN(96)));

    const zero = new BN(S0.sqrtPriceX96).pow(new BN("2"))
    const one = (new BN("2").pow(new BN("96")))

    console.log(zero.div(one).toString());
    console.log('\n');
    
    // sqrt(price * 2^96) = sqrtPriceX96 
    let price = 185873
    const nogger = new BN(price).mul(one)
    const sqrtPriceX96 = new BN(sqrt(BigInt(nogger.toString())).toString()).div(new BN(100))

    // sqrtPriceX96 = sqrt(1.0001^tick) * 2^96
    let tick = 60
    let two = (1.0001 ** tick) ** 0.5
    let dog = (new BN(two * 1000000000000000).mul(new BN(one))).div(new BN(1000000000000000))
    
    // let dog = (new BN(((1.0001 ** tick) ** 0.5) * 1000000000000000).mul(new BN(one))).div(new BN(1000000000000000))
    
    console.log(sqrtPriceX96.toString());
    console.log(dog.toString());
    
    // while (parseInt(sqrtPriceX96.toString()) <= parseInt((new BN(((1.0001 ** tick) ** 0.5) * 1000000000000000).mul(new BN(one))).div(new BN(1000000000000000)).toString())) {
    //     // console.log(sqrtPriceX96.toString());
    //     tick += 60
    // }
...
async function bhopa() {
    // 0.053800173
    const token0Decimals = 6
    const token1Decimals = 18

    // (1.0001^tick)/((10^token0Decimals) / (10^token1Decimals)) = Token0 price in relation to Token1
    const xi = ((1.0001 ** 201043) / (10 ** token0Decimals)) / (10 ** token1Decimals)
    console.log((1.0001 ** 201043) / (10 ** token0Decimals))

    // let tick = 0
    // while ((((1.0001 ** tick) / (10 ** token0Decimals)) / (10 ** token1Decimals)) <= 0.00053800173) {
    //     tick += 60
    // }
    // console.log(tick);
    // console.log(((1.0001 ** tick) / (10 ** token0Decimals)) / (10 ** token1Decimals));

    // console.log('\n');

    // tick = 0
    // while ((((1.0001 ** tick) / (10 ** token1Decimals)) / (10 ** token0Decimals)) <= 1858.73) {
    //     tick += 60
    // }
    // console.log(tick);
    // console.log(((1.0001 ** tick) / (10 ** token1Decimals)) / (10 ** token0Decimals));
}
// bhopa()

function randi() {
    // const priceQuote = 0.000556508585
    const priceQuote = 1803.55

    const token0Decimals = 6
    const token1Decimals = 18

    // const main = (new BN(10).pow(new BN(token1Decimals))).sub(new BN(10).pow(new BN(token0Decimals)))
    const main = 10 ** (token1Decimals - token0Decimals)
    const price = main * priceQuote

    // price = 1.0001 ^ tick
    // tick = ln(price) / ln(1.0001)
    // tick = ln(price) / 0.000099995

    var integerPrice = new BN(price).toString();
    var lnPrice = Math.log(integerPrice).toFixed(9);

    console.log(price, lnPrice);
    
    const tick0 = parseFloat(lnPrice) / 0.000099995
    const tick = Math.round(tick0/60)*60
...
function getTicksDefault() {
    ...
    var integerPrice0 = new BN(decimals).mul(new BN(price0)).toString();
    var lnPrice0 = Math.log(integerPrice0).toFixed(9);

    var integerPrice1 = new BN(decimals).mul(new BN(price1)).toString();
    var lnPrice1 = Math.log(integerPrice1).toFixed(9);

    
    const tick00 = parseFloat(lnPrice0) / 0.000099995
    const tick0 = Math.round(tick00/tickSpacing)*tickSpacing

    const tick01 = parseFloat(lnPrice1) / 0.000099995
    const tick1 = Math.round(tick01/tickSpacing)*tickSpacing

    console.log(tick0);
    console.log(tick1);
}

function getTicks(price) {
    const tickSpacing = 60
    ...
    // var integerPrice = new BN(decimals).mul(new BN(price)).toString();
...
getTicks(0.0009781185)

// price([poolABI, pool]).then((result) => {console.log(result);})
